...
export default function AdminDashboard({ records, tracks, names, preview, api, refresh, go }) {
    ...
    useEffect(() => { if (!selected)
        ...
        const controls = Array.from(dialog.current?.querySelectorAll('button:not(:disabled),a[href],input,select,textarea,[tabindex="0"]') || []).filter(element => element.getClientRects().length > 0);
        if (!controls?.length)
            return;
    ...
    async function approve(row, single, status = 'approved') {
        ...
        const feedbackFor = id => (feedback[id] || '').trim();
        ...
        if (ids.some(id => feedbackFor(id).length > 0 && feedbackFor(id).length < 10)) {
            setNotice('Review feedback must contain at least 10 characters.');
            locked.current = false; setBusy(false); return;
        }
        try {
            if (preview)
                setSamples(old => ({ ...old, applications: old.applications.map(a => a.id === row.id && a.status === 'pending' ? { ...a, status: 'approved' } : a), submissions: old.submissions.map(s => ids.includes(s.id) ? { ...s, status, feedback: feedbackFor(s.id) } : s) }));
            else {
                if (row.status === 'Application pending')
                    await api('approve_application', { id: row.id });
                else
                    for (const id of ids)
                        await api('review', { id, status, feedback: feedbackFor(id) || 'Project evidence reviewed and approved by the program administrator.' });
                await refresh();
            }
            setNotice(status === 'rejected' ? `Changes requested for ${row.student_name}.` : row.status === 'Application pending' ? `${row.student_name}’s application approved.` : single ? `Project approved for ${row.student_name}.` : row.current === 'Advanced' ? `${row.student_name}’s final level approved.` : `${row.student_name} promoted to ${levels[levels.indexOf(row.current) + 1]}.`);
        }
        catch (e) {
    ...
    const action = (r) => r.status === 'Application pending' ? <button className="ad-promote" disabled={busy} onClick={() => approve(r)}>Approve application</button> : <button className={r.pending.length ? 'ad-promote' : 'ad-view'} onClick={() => setSelected(r.id)}>{r.pending.length ? 'Review projects' : 'View details'} <ArrowUpRight size={13}/></button>;
    return <div className="ad-shell">
  <aside className="ad-sidebar"><Brand admin/><div className="ad-workspace"><span className="ad-workspace-icon"><Layers3 size={17}/></span><div>Admin workspace<small>Learning management</small></div><ShieldCheck size={14}/></div><span className="ad-nav-label">WORKSPACE</span><nav aria-label="Admin navigation"><button className={tab !== 'pending' ? 'chosen' : ''} onClick={() => setTab('all')}><Users size={17}/>User management <span>{rows.length}</span></button><button className={tab === 'pending' ? 'chosen' : ''} onClick={() => setTab('pending')}><Clock3 size={17}/>Pending reviews <span className="ad-nav-count">{pendingCount}</span></button><button onClick={() => go('/admin/manage')}><BookOpen size={17}/>Curriculum <ArrowUpRight size={13}/></button></nav><div className="ad-sidebar-bottom"><div className="ad-tip"><span><Sparkles size={16}/> Small steps. Real growth.</span><p>Every review brings a learner one step closer to what’s next.</p><button onClick={() => setTab('pending')}>Review submissions <ArrowRight size={14}/></button></div><a className="ad-back" href="/dashboard"><LayoutGrid size={16}/>Back to my workspace<ArrowUpRight size={13}/></a><div className="ad-account"><span className="ad-avatar">SA</span><div>Samkov admin<small>Program administrator</small></div><ShieldCheck size={16}/></div></div></aside>
  <div className="ad-main"><div className="ad-topbar"><div>Workspace <ChevronRight size={13}/><strong>User management</strong></div><span className="ad-environment"><i />{preview ? 'Preview workspace' : 'Admin workspace'}</span></div><main className="ad-content"><div className="ad-heading"><div><div className="ad-eyebrow">YOUR LEARNERS, AT A GLANCE</div><h1>User management<span>{rows.length}</span></h1><p>A little guidance. A lot of potential. Manage every learner’s next step.</p></div><button className="ad-button" aria-label="Export users" onClick={exportRows}><ArrowDownToLine size={15}/>Export users</button></div>
   <div className="ad-stats">{[{ label: 'Registered users', value: preview ? 'Demo' : analytics?.total_users, icon: Users, note: 'Enabled accounts, including admins', kind: '' }, { label: 'Active now', value: preview ? 'Demo' : analytics?.active_users, icon: GraduationCap, note: 'Signed in · seen in the last 5 minutes', kind: 'green' }, { label: 'Page views today', value: preview ? 'Demo' : analytics?.views_today, icon: Clock3, note: 'Page loads and navigation · UTC', kind: 'amber' }, { label: 'Total page views', value: preview ? 'Demo' : analytics?.total_views, icon: Layers3, note: 'Recorded since tracking was enabled', kind: 'purple' }].map(s => <div className={`ad-stat ${s.kind}`} key={s.label}><div>{s.label}<s.icon size={17}/></div><strong>{s.value ?? '—'}</strong><small><span className="ad-stat-dot"/>{s.note}</small></div>)}</div>
   <p className="ad-live-note" role="status">{preview ? 'Design preview · sample learners below' : records.account_sync_error || analyticsError || (analytics ? `Updated ${new Date(analytics.updated_at).toLocaleTimeString()} · refreshes every 15 seconds` : 'Loading live statistics…')}</p>
   ...
   <section className="ad-sheet" aria-label="Learner directory"><div className="ad-sheet-tabs" role="tablist" aria-label="Filter users by status">{[['all', 'All users', rows.length], ['pending', 'Pending reviews', pendingCount], ['active', 'Active', rows.filter(r => r.status === 'Active').length], ['completed', 'Completed', rows.filter(r => r.status === 'Completed').length]].map(([id, label, count]) => <button key={id} role="tab" aria-selected={tab === id} className={tab === id ? 'selected' : ''} onClick={() => setTab(String(id))}>{label}<span>{count}</span></button>)}<span className="ad-sheet-caption"><Layers3 size={13}/>Learner directory</span></div>
    ...
    <div className="ad-table-scroll"><table><thead><tr><th className="ad-number">#</th><th aria-sort={sort ? 'ascending' : 'none'}><button onClick={() => setSort(!sort)} aria-label={sort ? 'Use original order' : 'Sort by name'}>Name <ChevronsUpDown size={12}/></button></th>{['Email', 'Level', 'Current course', 'Progress', 'Recent project', 'Project link', 'Status', 'Actions'].map(s => <th key={s}>{s}</th>)}</tr></thead><tbody>{visible.map((r, i) => <tr key={r.id} className={selected === r.id ? 'ad-row-selected' : ''}><td className="ad-number">{String((safePage - 1) * 8 + i + 1).padStart(2, '0')}</td><td><button className="ad-person" onClick={() => setSelected(r.id)}>{avatar(r.student_name, i)}<strong>{r.student_name}{r.username && <small className="ad-username">@{r.username}</small>}</strong></button></td><td className="ad-email" title={r.email}>{r.email}</td><td><span className={`ad-level level-${Math.max(0, levels.indexOf(r.current))}`}><span className="ad-level-bars" aria-hidden="true">▂▅▇</span>{r.current === 'Not started' ? '—' : 'L' + (levels.indexOf(r.current) + 1)}<span className="ad-level-name"> · {r.current}</span></span></td><td>{r.course}</td><td><div className="ad-progress"><span><i style={{ width: r.progress + '%' }}/></span><b>{r.progress}%</b></div></td><td className="ad-project" title={r.projects[r.latest?.project_index]}>{r.projects[r.latest?.project_index] || 'No submissions yet'}</td><td>{r.latest?.github_url ? <a className="ad-project-link" href={r.latest.github_url} target="_blank" rel="noreferrer">View project <ExternalLink size={11}/></a> : <span className="ad-dash">—</span>}</td><td>{badge(r.status)}</td><td>{action(r)}</td></tr>)}</tbody></table>{!filtered.length && <div className="ad-empty"><Search size={25}/><h3>No users found</h3><p>Try another name or adjust your filters.</p><button className="ad-button" onClick={() => { setQuery(''); setCourse('all'); setLevel('all'); setTab('all'); }}>Clear filters</button></div>}</div>
    <div className="ad-mobile-list">{visible.map((r, i) => <article className="ad-learner-card" key={r.id} aria-label={r.student_name + ' learner record'}>
      <div className="ad-card-heading"><button className="ad-person" onClick={() => setSelected(r.id)}>{avatar(r.student_name, i)}<span><strong>{r.student_name}</strong>{r.username && <small className="ad-username">@{r.username}</small>}</span></button>{badge(r.status)}</div>
      <p className="ad-card-email">{r.email}</p><h3>{r.course}</h3>
      <div className="ad-card-progress"><span>{r.current}</span><strong>{r.progress}% complete</strong></div><div className="ad-card-track"><i style={{width: r.progress + '%'}}/></div>
      <div className="ad-card-project"><span>Latest project</span><strong>{r.projects[r.latest?.project_index] || 'No submissions yet'}</strong></div>
      <div className="ad-card-actions">{r.latest?.github_url && <a className="ad-project-link" href={r.latest.github_url} target="_blank" rel="noreferrer">View project <ExternalLink size={14}/></a>}{action(r)}</div>
    </article>)}{!filtered.length && <div className="ad-empty"><Search size={25}/><h3>No users found</h3><p>Try another name or adjust your filters.</p><button className="ad-button" onClick={() => { setQuery(''); setCourse('all'); setLevel('all'); setTab('all'); }}>Clear filters</button></div>}</div>
    <div className="ad-pagination"><span>Showing <b>{filtered.length ? (safePage - 1) * 8 + 1 : 0}–{Math.min(safePage * 8, filtered.length)}</b> of <b>{filtered.length}</b> users</span><div><button aria-label="Previous page" disabled={safePage === 1} onClick={() => setPage(safePage - 1)}><ChevronLeft size={14}/></button>{Array.from({ length: pageCount }, (_, i) => i + 1).filter(n => n === 1 || n === pageCount || Math.abs(n - safePage) <= 1).map(n => <button aria-label={`Page ${n}`} aria-current={n === safePage ? 'page' : undefined} className={safePage === n ? 'current' : ''} key={n} onClick={() => setPage(n)}>{n}</button>)}<button aria-label="Next page" disabled={safePage === pageCount} onClick={() => setPage(safePage + 1)}><ChevronRight size={14}/></button></div></div>
   </section><div className="ad-footnote"><span><ShieldCheck size={13}/>Thoughtful reviews. Meaningful progress.</span><span>{preview ? 'Sample data · Changes last until you leave this page' : 'Only approved evidence unlocks the next level'}</span></div>
  </main></div>
